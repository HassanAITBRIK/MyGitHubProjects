function [W1,B1,W2,B2,W3,B3,W4,B4,L,LR,HLayer] = mlp4atrain(Base, Target, W1, B1, W2, B2, W3, B3, W4, B4, lr, n, err_glob, freqplot)
%
% MLP4ATRAIN
%
% SYNTAXE :
%
% [W1,B1,W2,B2,W3,B3,W4,B4,L,LR,HLayer]=mlp4atrain(Base, Target, W1, B1, W2, B2, W3, B3, W4, B4 [, lr, n, err_glob] [, freqplot]);
%
...
if (nargin<13 || nargin>14) && nargin~=10 && nargin ~=11
   error('[MLP4ATRAIN] Usage : [W1,B1,W2,B2,W3,B3,W4,B4,L,LR] = mlp4atrain(Base, Target, W1, B1, W2, B2, W3, B3, W4, B4 [, lr, n, err_glob] [, freqplot])');
end;

if nargin==10 || nargin==13, graph_mode = 0; freqplot = n+1;	% pas d'affichage par défaut
else graph_mode = 1;
   if nargin==11, freqplot = lr; end;                        % arg N. 11 = freq. d'affichage   
end;

if nargin<13
   disp('MLP4ATRAIN : Paramètres de l''apprentissage');
...
elseif length(lr) ~= 4
   error('[MLP4ATRAIN] Arguments <lr> incorrecte (lr=[lr0, lr_dec, lr_inc, delta]'); 
end;
...
if input_nbr~=n_in
  error('[MLP4ATRAIN] Défaut de cohérence dans les arguments : W1 et Data'); end;
if size(B1)~=[n_cell 1]
  error('[MLP4ATRAIN] Défaut de cohérence dans les arguments : W1 et B1'); end;
if size(W4)~=[output_nbr n_cell]
  error('[MLP4ATRAIN] Défaut de cohérence dans les arguments : W4 et Class'); end;
if size(B4)~=[output_nbr 1]
  error('[MLP4ATRAIN] Défaut de cohérence dans les arguments : W4 et B4'); end;
