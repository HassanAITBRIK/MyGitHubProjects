function [W1,B1,W2,B2,W3,B3,L,LR,HLayer] = mlp3atrain(Base, Target, W1, B1, W2, B2, W3, B3, lr, n, err_glob, freqplot)
%
% MLP2ATRAIN
%
% SYNTAXE :
%
% [W1,B1,W2,B2,W3,B3,L,LR,HLayer]=mlp2atrain(Base, Target, W1, B1, W2, B2, W3, B3 [, lr, n, err_glob] [, freqplot]);
%
% Apprentissage des poids d'un Perceptron MultiCouche par
...
% VALEURS DE RETOUR :
%
% W1,B1,W2,B2,W3,B3 : les poids calculés à l'issue de l'apprentissage
% L              	: coût quadratique
...
if (nargin<11 || nargin>12) && nargin~=8 && nargin ~=9
   error('[MLP3ATRAIN] Usage : [W1,B1,W2,B2,W3,B3,L,LR] = mlp3atrain(Base, Target, W1, B1, W2, B2, W3, B3 [, lr, n, err_glob] [, freqplot])');
end;

if nargin==8 || nargin==11, graph_mode = 0; freqplot = n+1;	% pas d'affichage par défaut
else graph_mode = 1;
   if nargin==9, freqplot = lr; end;                        % arg N. 9 = freq. d'affichage   
end;

if nargin<11
   disp('MLP3ATRAIN : Paramètres de l''apprentissage');
...
elseif length(lr) ~= 4
   error('[MLP3ATRAIN] Arguments <lr> incorrecte (lr=[lr0, lr_dec, lr_inc, delta]'); 
end;
...
if input_nbr~=n_in
  error('[MLP3ATRAIN] Défaut de cohérence dans les arguments : W1 et Data'); end;
if size(B1)~=[n_cell 1]
  error('[MLP3ATRAIN] Défaut de cohérence dans les arguments : W1 et B1'); end;
if size(W3)~=[output_nbr n_cell]
  error('[MLP3ATRAIN] Défaut de cohérence dans les arguments : W3 et Class'); end;
if size(B3)~=[output_nbr 1]
  error('[MLP3ATRAIN] Défaut de cohérence dans les arguments : W3 et B3'); end;
...
% --- rétropropagation :
			Delta3 = -2*E.*sigmop(V3);
%			Delta3 = -2*E;
            D3 = Delta3*S2';

			Delta2 = (W3'*Delta3).*sigmop(V2);
			D2 = Delta2*S1';
...
% --- Sauvegarde des poids courants et modif. des poids :
			TW1 = W1; TB1 = B1; TW2 = W2; TB2 = B2; TW3 = W3; TB3 = B3;
			TL = L(it);
